...
{	
	...
	BasicConfigurator.configure();
        ...
        JFrame jf = new JFrame("Keyword-based Resume Scanner (V1)"); jf.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); jf.setSize(600, 800);
	JPanel jpanel = new JPanel(); jpanel.setBackground(Color.getHSBColor(25, 130, 95)); jpanel.setLayout(null);
	...
	ButtonToDisplay.addActionListener(new ActionListener()
	{
		...
		try
	        {   // Taking whitespace-separated keywords as input (from the first text field; second one's for the directory) and thereafter dividing the input string (splits based on blank space) into an array of keywords:
	            String keywordString = InputTextFieldOne.getText(); String[] keywords = keywordString.split(" ");
		    String directory = InputTextFieldTwo.getText(); File dir = new File(directory); File[] fileslist = dir.listFiles(new FileFilterer());			
			...
			{   
				...
				FileInputStream fis = new FileInputStream(f.getAbsolutePath()); HWPFDocument document = new HWPFDocument(fis); extractor = new WordExtractor(document);
			...
			{   
				...
				{
				    PDFTextStripperByArea stripper = new PDFTextStripperByArea(); stripper.setSortByPosition(true); PDFTextStripper tStripper = new PDFTextStripper();
